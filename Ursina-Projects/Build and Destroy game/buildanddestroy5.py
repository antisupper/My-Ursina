...
def input(key):
    ...
    if key in key_texture:
        block_texture = key_texture[key]

    if ground.hovered:
        if key == 'left mouse down':
            block = Entity(model='cube',color=color.red.tint(0.4),texture=block_texture, scale=(2,2,2),world_position=mouse.world_point, collider = 'box' )
            blocks.append(block)
            
    for block in blocks: 
            if block.hovered == True: 
                if key == 'left mouse down': 
                    block = Entity(model='cube',color=color.red.tint(.4), texture=block_texture,scale=(2,2,2),position=mouse.world_point,collider = 'box' )
                    blocks.append(block)
